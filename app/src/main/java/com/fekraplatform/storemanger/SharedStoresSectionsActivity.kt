...
import com.fekraplatform.storemanger.models.SectionStoreCategory
...
class SharedStoresSectionsActivity : ComponentActivity() {
    private val sectionStoreCategories = mutableStateOf<List<SectionStoreCategory>>(listOf())
    private val sections = mutableStateOf<List<Section>>(listOf())
    val stateController = StateController()
    val requestServer = RequestServer(this)
    val isShowAddSection = mutableStateOf(false)

    lateinit var storeCategory1: StoreCategory1
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (str != null) {
            try {
                storeCategory1 = MyJson.IgnoreUnknownKeys.decodeFromString(str)
            }catch (e:Exception){
                finish()
            }
...
//        startActivity(intent)
//    }
private fun goToSections(sectionStoreCategory: StoreCategorySection) {
    val intent = Intent(
        this,
        SharedStoresNestedSectionsActivity::class.java
    )
    intent.putExtra("sectionStoreCategory", MyJson.MyJson.encodeToString(sectionStoreCategory))
    startActivity(intent)
}
fun read() {
    ...
    val body = MultipartBody.Builder()
        ...
        .addFormDataPart("storeCategory1Id", storeCategory1.id.toString())
        .build()
...
fun readSections() {
        ...
        val body = MultipartBody.Builder()
            .setType(MultipartBody.FORM)
            .addFormDataPart("category1Id", storeCategory1.categoryId.toString())
            .addFormDataPart("storeId", "1")
            .build()
...
private fun add(sectionId: String) {
        ...
        requestServer.request(body,"${U1R.BASE_URL}${U1R.VERSION}/${U1R.TYPE}/addSectionStoreCategory",{code,fail->
            ...
            sectionStoreCategories.value += result
                        ...
                        Card(Modifier.padding(8.dp)) {
                            ...
                            if (expanded)
                                sections.value.filterNot { sectionItem ->
                                    sectionStoreCategories.value.any { storeCategory ->
